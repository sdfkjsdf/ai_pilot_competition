#include "LadyLuck/guidance/dbfm/DbfmBreakLoad.hpp"
...
// Frozen NumPy 1.26.4/MKL authority evaluates the three-element reduction as
// (x0 + x2) + x1.  Preserve that association here;
// the conventional left-associated helper differs by up to four ULP in the
// non-planar BREAK allocation and then perturbs the RollGV2 cross-check.
double PythonDot3(
...
bool IsBreakLabel(const std::string& label) noexcept
{
    return label == "BREAK"
        || label == "GUN_DEFENSE_HORIZONTAL_BREAK"
        || label == "GUN_DEFENSE_TRACKING_JINK";
}
...
LadyLuck::Result<LadyLuck::TacticalCommand> Passthrough(
    const LadyLuck::TacticalCommand& command)
{
...
Result<DbfmBreakLoadReference> BuildDbfmBreakLoadReference(
    ...
    const double instantaneous_load_limit_g) noexcept
{
    Result<DbfmBreakLoadReference> result{};
    const Status structure = ValidateTacticalCommandStructure(command);
    if (!structure.ok())
    {
        result.status = structure;
        return result;
    }
    if (!IsBreakLabel(command.behavior_label)
        || command.mode_label != "DBFM"
    ...
    const DbfmBreakLoadConfig& config)
{
    ...
    candidate.direct_acceleration_ned_mps2.value =
        reference.value.admitted_acceleration_ned_mps2;
    ...
    Status& status) noexcept
{
    ...
    // A selected BREAK owner and a declared-ready capability receipt form the
    // mandatory contract.  Contradictions in that contract remain faults;
    // only the optional load-vector geometry below is allowed to decline and
    // preserve the already-valid BREAK command.
    ...
    {
        // Finite geometry can legitimately be outside the optional BREAK-load
        // materializer's domain (zero horizontal velocity axis, zero lateral
        // force direction, or vertical force consuming the instantaneous-load
        // budget).  The TacticalCommand overload already retains the base
        // BREAK; do the same here so normal overlay non-admission cannot
        // suppress the frame command.  This deliberately strengthens the
        // literal Python helper's assumed-domain exception boundary.
        output = command;
        status = Status{};
        return;
    }
    if (status.code != StatusCode::Ok)
    {
        // Non-finite arithmetic and internal contract failures are not normal
        // load-overlay non-admission. Preserve their typed status so the
        // enclosing production owner can route them through its fault policy.
        return;
    }
